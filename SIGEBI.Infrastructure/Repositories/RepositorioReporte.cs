...
namespace SIGEBI.Infrastructure.Repositories
{
    public class RepositorioReporte : IRepositorioReporte
    {
        ...
        public RepositorioReporte(SIGEBIDbContext context, ILogger<RepositorioReporte> logger)
        {
            _context = context;
            _logger = logger;
        }
        ...
        public async Task<ReporteInventarioResponse> ObtenerReporteInventarioAsync()
        {
            try
            {
                _logger.LogInformation("Generando reporte de inventario.");
            ...
            {
                _logger.LogError(ex, "Error al generar el reporte de inventario: {Message}", ex.Message);
                throw;
            }
        }

        public async Task<ReporteUsoCatalogoResponse> ObtenerReporteUsoCatalogoAsync(DateTime fechaInicio, DateTime fechaFin)
        {
            try
            {
                _logger.LogInformation("Generando reporte de uso de catálogo desde {Inicio} hasta {Fin}", fechaInicio, fechaFin);
                ...
                var solicitudes = await _context.Solicitudes
                    ...
                    .AsNoTracking()
                    .Where(s => s.FechaSolicitud >= inicio && s.FechaSolicitud < finExclusiva)
                    .Select(s => new
                    {
                        RecursoBibliograficoId = s.Ejemplar != null && s.Ejemplar.RecursoBibliografico != null
                ...
                int ejemplaresDisponibles = recursos.Sum(r => r.Ejemplares.Count(e => e.Estado == EstadoEjemplar.Disponible));

                decimal disponibilidadPromedio = totalEjemplares > 0
                    ? Math.Round((decimal)ejemplaresDisponibles / totalEjemplares * 100, 2)
                    : 0;
            ...
            {
                _logger.LogError(ex, "Error al generar el reporte de uso de catálogo: {Message}", ex.Message);
                throw;
            }
        }

        public async Task<ReportePrestamoResponse> ObtenerReportePrestamoAsync(DateTime fechaInicio, DateTime fechaFin)
        {
            try
            {
                _logger.LogInformation("Generando reporte de préstamos desde {Inicio} hasta {Fin}", fechaInicio, fechaFin);
                ...
                var prestamos = await _context.Prestamos
                    ...
                    .AsNoTracking()
                    .Where(p => p.FechaInicio >= inicio && p.FechaInicio < finExclusiva)
                    .ToListAsync();

                int totalPrestamos = prestamos.Count;
                int devueltosATiempo = prestamos.Count(p => p.Devolucion != null && p.Devolucion.FechaDevolucion <= p.FechaLimite);

                int vencidos = prestamos.Count(p =>
                    (p.Devolucion != null && p.Devolucion.FechaDevolucion > p.FechaLimite) ||
                    (p.Devolucion == null && DateTime.UtcNow > p.FechaLimite));
                ...
                var detalles = prestamos
                    .Select(p => new DetallePrestamoReporteResponse
                    {
                        ...
                        RecursoBibliograficoId = p.Ejemplar?.RecursoBibliografico?.RecursoBibliograficoId ?? 0,
                        TituloRecurso = p.Ejemplar?.RecursoBibliografico?.Titulo ?? "Sin título",
                        IdentificadorEjemplar = p.Ejemplar?.Identificador ?? "Sin identificador",
            ...
            {
                _logger.LogError(ex, "Error al generar el reporte de préstamos: {Message}", ex.Message);
                throw;
            }
        }

        public async Task<ReportePenalizacionesResponse> ObtenerReportePenalizacionesAsync(DateTime fechaInicio, DateTime fechaFin)
        {
            try
            {
                _logger.LogInformation("Generando reporte de penalizaciones desde {Inicio} hasta {Fin}", fechaInicio, fechaFin);
                ...
                var penalizaciones = await _context.Penalizaciones
                    .Include(p => p.Usuario)
                    .AsNoTracking()
                    .Where(p => p.FechaGeneracion >= inicio && p.FechaGeneracion < finExclusiva)
                    .ToListAsync();

                int total = penalizaciones.Count;
                int activas = penalizaciones.Count(p => p.Estado == EstadoPenalizacion.Activa);
                int resueltas = penalizaciones.Count(p => p.Estado == EstadoPenalizacion.Resuelta);
                int totalDiasRetraso = penalizaciones.Sum(p => p.DiasRetraso);
                decimal montoTotal = penalizaciones.Sum(p => p.MontoMora);
                decimal montoActivo = penalizaciones.Where(p => p.Estado == EstadoPenalizacion.Activa).Sum(p => p.MontoMora);
                decimal montoResuelto = penalizaciones.Where(p => p.Estado == EstadoPenalizacion.Resuelta).Sum(p => p.MontoMora);
                ...
                var porTipoUsuario = penalizaciones
                    ...
                    .Select(grupo => new PenalizacionPorTipoUsuarioResponse
                    {
                        ...
                        Activas = grupo.Count(p => p.Estado == EstadoPenalizacion.Activa),
                        Resueltas = grupo.Count(p => p.Estado == EstadoPenalizacion.Resuelta),
                        MontoTotal = grupo.Sum(p => p.MontoMora)
                    })
            ...
            {
                _logger.LogError(ex, "Error al generar el reporte de penalizaciones: {Message}", ex.Message);
                throw;
            }
        }
    }
